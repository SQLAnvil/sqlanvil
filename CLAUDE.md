...
cli/          CLI: cli/index.ts registers one module per command from cli/commands/ (shared flags in
              cli/common_options.ts, workflow_settings overrides in cli/project_config_options.ts);
              per-adapter glue in cli/api/dbadapters/ — bigquery, postgres, supabase, mysql
...
- **CLI:** `cli/commands/{run,test,validate}_command.ts` branch the adapter on `projectConfig.warehouse ∈ {bigquery, postgres, supabase}`; `credentials.ts` validates `PostgresConnection`.
...
**Run the integration tests** — `./tools/postgres/run-postgres-db.sh` boots a Postgres+pgvector container; run the specs with **native bazel** against `127.0.0.1`, passing env with `--test_env` (the `:postgres.spec`/`:supabase.spec` targets are runnable; the `*_tests` suffixes are compile-only macros). Native runs validated for all five specs on 2026-07-04 — `docker-bazel` is no longer required (if you do use it, connect via `host.docker.internal` instead):

```bash
PG_HOST=127.0.0.1 PG_PORT=5432 PG_USER=postgres PG_PASSWORD=password PG_DATABASE=postgres \
  bazel test //tests/integration:postgres.spec \
...
- **File import (`type: "import"`).** The symmetric inverse of `type: "export"` (1.8.0): loads a
  ...
  format, overwrite }`; `location` is the verbatim source file/glob/URI (NOT a derived filename). On
  **BigQuery** it's native `LOAD DATA OVERWRITE|INTO … FROM FILES(…)` (gs:// only). On
  **Postgres/Supabase** the runner loads it via the DuckDB **bridge** (read-write `ATTACH`, then
  `CREATE TABLE pg."s"."t" AS SELECT * FROM read_parquet(…)` to replace, or `INSERT INTO … SELECT` to
  append). **MySQL** throws (not supported yet, like export). Data loads as-is (no transform — shape
  it downstream); `overwrite` defaults true (drop+create), `false` appends to an existing table.
  Honest framing: this is *file movement into your warehouse*, not "your warehouse reads Parquet
  live" — preserves the single-write-warehouse model. `validate` treats imports as SKIPPED (no
  planner query; file columns unknown pre-run), so a model reading an imported table is BLOCKED.
  Code: `core/actions/import.ts` (config-only `Import` builder), `cli/api/dbadapters/duckdb_import.ts`
  (`buildImportSql`/`runDuckdbImport`; shares `buildSecretSql`/`schemeOf`/`buildAttachSql` with
  `duckdb_export.ts` — `buildAttachSql(pg, {readOnly:false})` for the RW attach), per-warehouse
  `createImportTasks` on each `*_execution_sql.ts`, `run.ts` `task.type==="import"` dispatch
  (`duckdbImport` seam). Integration-tested in `postgres.spec` (live export→import round-trip,
  overwrite + append). Design/plan: `sqlanvil-private/planning/{specs/2026-06-24-duckdb-warehouse-adapter-design.md,plans/2026-06-26-duckdb-import-action-plan.md}`.
- **Queryable artifacts (`sqlanvil query` / `inspect`).** `compile` writes a catalog and `run` adds a run-history entry as **Parquet** under `<projectDir>/target/` (`catalog/{actions,dependencies,columns}.parquet`, `runs/run_<ts>.parquet`) — best-effort, never fails compile/run; `--no-artifacts` disables. `sqlanvil query "<sql>"` runs SQL over them (views: `actions`, `dependencies`, `columns`, `runs`) via the bundled DuckDB; `sqlanvil inspect` prints counts-by-type + latest-run summary + recent failures. **`sqlanvil docs`** renders a self-contained HTML catalog (models, types, tags, columns, object-level dependency edges, last-run status) to `target/docs/index.html` — no column-level lineage, no graph viz, no server/CDN (`cli/api/commands/docs.ts`: pure `renderDocsHtml` + `buildDocsModel`). Inline CTE preview (assessment §4.4) is deferred to the VS Code extension (needs a warehouse connection). Code: `cli/api/commands/artifact_rows.ts` (pure compiledGraph/runResult → rows) + `artifacts.ts` (`writeArtifacts`/`safeWriteArtifacts`), `cli/api/dbadapters/duckdb_artifacts.ts` (`writeParquet` rows→temp JSON→`COPY`; `queryParquet` over `read_parquet` views), shared DuckDB conn in `cli/api/dbadapters/duckdb.ts` (`withDuckdb`, used by exports too), CLI `cli/commands/{query,inspect,docs}_command.ts` (shared `artifact_views.ts`) + the compile/run hooks in `compile_command.ts`/`run_command.ts`. Warehouse-agnostic (no warehouse connection needed to query). Tell users to gitignore `target/`. Integration-tested via `cli/api/commands/artifacts_integration_test.ts` (DuckDB round-trip, incl. empty rowsets).
- **`sqlanvil validate`** (Postgres/Supabase/MySQL via `EXPLAIN`, + BigQuery via dry-run) walks the compiled DAG in topological order and validates each model against the warehouse planner (`EXPLAIN`) without executing. It compiles into a timestamped shadow namespace (a validation `schemaSuffix` composed onto the project/env suffix) and, after each model passes, materializes an **empty** stub (`CREATE TABLE … WITH NO DATA` on Postgres; wrapped `LIMIT 0` on MySQL; views via `CREATE VIEW`) so downstream `${ref()}`s resolve; the shadow schema(s)/database(s) are dropped in a `finally`. Per-action result is **PASS / FAILURE / BLOCKED** (a model whose only problem is an upstream failure) **/ SKIPPED** (operations — not validated); any FAILURE or BLOCKED exits non-zero. Code: `cli/api/commands/validate.ts` (orchestrator) + `validate_graph.ts` (topo + classification), builders on each `*_execution_sql.ts` (`validationStubSql`/`createSchemaSql`/`dropSchemaCascadeSql`), CLI in `cli/commands/validate_command.ts` (its exported `runValidate` backs both the `validate` command and the `run --dry-run` alias in `run_command.ts`). **`run --dry-run` on Postgres/Supabase/MySQL now validates instead of executing** — previously it was a no-op that still applied changes while printing "no changes". BigQuery keeps its native server-side dry-run on `run --dry-run`; the `validate` *command* additionally supports BigQuery (shadow dataset of `LIMIT 0` stubs), live-verified against project `sqlanvil` (see Nice-to-haves for the BQ sweep caveat). Each run first **sweeps orphaned shadows** — `*sqlanvil_validate_<ts>` schemas/databases older than 1h, left by killed runs (`sweepOrphanShadows`, matched by the `sqlanvil_validate_` marker so real schemas are never touched).
