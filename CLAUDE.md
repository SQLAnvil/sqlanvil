...
- **`sqlanvil validate` fast-follows.** The command shipped for Postgres/Supabase/MySQL, and `run --dry-run` now delegates to it on those warehouses (see Things To Know). Remaining: **BigQuery** support (dry-run + a shadow dataset of `LIMIT 0` tables — the adapter's `validationStubSql`/`createSchemaSql`/`dropSchemaCascadeSql` currently throw on BQ); and an **orphan-shadow sweep** (drop stray `*__sqlanvil_validate_*` schemas from killed runs). Design + plan: `sqlanvil-private/planning/{specs,plans}/2026-06-24-validate-command-*`.
