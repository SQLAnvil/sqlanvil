...
- **`sqlanvil validate` — BigQuery support (only remaining fast-follow).** The command shipped for Postgres/Supabase/MySQL, `run --dry-run` delegates to it there, and orphaned shadow schemas are swept automatically (see Things To Know). Still TODO: **BigQuery** (dry-run + a shadow dataset of `LIMIT 0` tables — the adapter's `validationStubSql`/`createSchemaSql`/`dropSchemaCascadeSql` currently throw on BQ). Design + plan: `sqlanvil-private/planning/{specs,plans}/2026-06-24-validate-command-*`.
