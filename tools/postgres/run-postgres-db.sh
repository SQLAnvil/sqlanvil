...
echo ""
echo "Run the integration tests. Notes:"
echo "  - Tests run INSIDE the docker-bazel container, so reach this container as"
echo "    host.docker.internal (NOT localhost)."
...
echo ""
echo "  PG_HOST=host.docker.internal PG_PORT=5432 PG_USER=postgres PG_PASSWORD=password PG_DATABASE=postgres \\"
echo "    ./scripts/docker-bazel test //tests/integration:postgres.spec \\"
echo "    --test_env=PG_HOST --test_env=PG_PORT --test_env=PG_USER --test_env=PG_PASSWORD --test_env=PG_DATABASE \\"
echo "    --jobs=2 --local_ram_resources=2048"
echo ""
echo "  SUPABASE_HOST=host.docker.internal SUPABASE_PORT=5432 SUPABASE_USER=postgres SUPABASE_PASSWORD=password SUPABASE_DATABASE=postgres \\"
echo "    ./scripts/docker-bazel test //tests/integration:supabase.spec \\"
